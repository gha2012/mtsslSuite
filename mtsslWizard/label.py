...
class Label:
	...
	def extractAtomNamesFromInputFile(self, string):
		...
		counter += 2
		atomNames = re.split('x|-|=', string.replace("!", ""))
		string.replace("!", "")
	...
	def extractRotationInfoFromInputString(self, string):
		...
		for idx, character in enumerate(string):
			if character == "-" or character == "=" or character == "x":
				atomCounter += 1
			#print atomCounter
			if character == "-" or character == "=":
				i = 1
	...
	def generateEnsembleMulti(self, movingAtoms, environmentAtomInfo, numberToFind, maxNtries, name, fixChi3, vdWcutOff, maxClash, queue):
		try:
		...
		newRotamers = []
		resultsDictionary = {}
		#unbox environmentAtomInfos
		environmentAtomCoordinates = environmentAtomInfo[0]
		environmentAtomNames = environmentAtomInfo[1]
		environmentAtomResidueNames = environmentAtomInfo[2]
		...
		while found < numberToFind and ntries < maxNtries:
			...
			if not self.quickClash(movingAtoms[self.clashAtoms], environmentAtomCoordinates, vdWcutOff, maxClash):
				if not self.internalClash2(movingAtoms, refDist):
					...
					for idx, atomName in enumerate(movingAtomNames):
						if idx >= 0:
							...
							#find contacts with environment in three shells
							#thisAtom.indices['near'] = self.getAtomIndicesInShell(thisAtom.coordinate, environmentAtomCoordinates, 2.2, 3.0)
							#thisAtom.indices['med'] = self.getAtomIndicesInShell(thisAtom.coordinate, environmentAtomCoordinates, 3.0, 4.0)
							#thisAtom.indices['far'] = self.getAtomIndicesInShell(thisAtom.coordinate, environmentAtomCoordinates, 4.0, 5.0)
							
							#classify atom
							#for shell in ["near","med","far"]:
							#	for neighbourAtomName, resn in zip(environmentAtomNames[thisAtom.indices["%s"%shell]], environmentAtomResidueNames[thisAtom.indices["%s"%shell]]):
							#		if thisAtom.ishydrophobic(neighbourAtomName, resn):
							#			thisAtom.counts['%sHydrophobic'%shell]+=1
							#		elif thisAtom.isPolar(neighbourAtomName, resn):
							#			thisAtom.counts['%sPolar'%shell]+=1
							#		elif thisAtom.isCharged(neighbourAtomName, resn):
							#			thisAtom.counts['%sCharged'%shell]+=1
							#		else:
							#			print "Could not classify atom: %s of residue: %s" %(neighbourAtomName, resn)
							
							#add atom to rotamer
							newRotamer.atoms[atomName] = thisAtom
							
					#score rotamer
					#newRotamer.score(scoringWeights)
					#self.ensembles[name].rotamers.append(rotamer)
					newRotamers.append(newRotamer)
					found += 1
					#print found,
				else:
				...
				#print ".",
			ntries += 1
		resultsDictionary[uniqueId] = newRotamers
		queue.put(resultsDictionary)
